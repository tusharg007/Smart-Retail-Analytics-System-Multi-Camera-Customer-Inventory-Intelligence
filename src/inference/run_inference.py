...
class VideoProcessor:
    ...
    def process_frame(self, frame):
        """Process single frame for person detection and tracking"""
        ...
        if len(results) > 0 and results[0].boxes is not None:
            ...
            for box in boxes:
                ...
                detections.append({
                    ...
                    'class': 'person',
                    'track_id': f"person_{track_id}" if track_id is not None else "unknown"
                })
    ...
    def process_video(self, video_path, output_path=None, show_display=False):
        ...
        while cap.isOpened():
            ...
            tracked_detections = self.process_frame(frame)
            ...
            analytics_data['frame_detections'].append({
                'frame': frame_count,
                'people_count': len(tracked_detections),
                'detections': tracked_detections
            })
            analytics_data['people_count_over_time'].append(len(tracked_detections))
            ...
            if frame_count % 30 == 0:
                ...
                print(f"Progress: {progress:.1f}% | FPS: {current_fps:.1f} | People: {len(tracked_detections)}")
